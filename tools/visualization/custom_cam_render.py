...
class CustomCamRender:
    ...
    def render(
        ...
        # Ours
        # self.draw_planning_pred(data, result, color=(0.5, 0, 0.5))
        self.draw_planning_pred(data, result, colormap="autumn")
        # SparseDrive
        # self.draw_planning_pred(data, spd_result, color=(0, 0, 1))
        self.draw_planning_pred(data, spd_result, colormap="winter")
        if self.plot_choices["cam_planning_gt"]:
    ...
    def draw_planning_gt(self, data):
        ...
        # self._render_traj(traj_points, cam_intrinsic, j=1, color=(1, 0.5, 0))
        self._render_traj_custom(traj_points, cam_intrinsic, j=1, colormap="Purples")
        
    def draw_planning_pred(self, data, result, colormap):
        ...
        traj_points = plan_traj @ extrinsic[:3, :3] + trans
        # self._render_traj(traj_points, cam_intrinsic, j=1, color=color)
        self._render_traj_custom(traj_points, cam_intrinsic, j=1, colormap=colormap)

    def _render_traj_custom(self, traj_points, cam_intrinsic, j, colormap="autumn", s=150, points_per_step=10):
        ...
        dot_colors = dot_colors[in_range_mask]
        self.axes[j // 3, j % 3].scatter(traj_points[0], traj_points[1], color=dot_colors, s=s)
