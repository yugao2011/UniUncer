...
class BEVRender:
    ...
    def render(
        ...
        self.reset_canvas(render_gt=True)  # 这里绘制gt
        self.draw_detection_gt(data)  # 只绘制detection时只有检测到的车
        ...
        self.reset_canvas()  # 这里绘制prediction的结果
        ...
        self.draw_planning_pred(data, result, top_k=self.plot_choices["planning_top_k"])
        self._render_sdc_car()
    ...
    def draw_planning_gt(self, data):
        ...
        # print('masks shape:', masks.shape)  shape为（6，），表明未来3s 6帧的mask结果
        if masks[0] != 0:
            plan_traj = data["gt_ego_fut_trajs"][masks]  # 表示未来真实的轨迹
            # print('plan_traj shape:', plan_traj.shape)  （6，2）第一维表示未来6帧，2表示xy坐标
            ...
            # print('plan_traj shape:', plan_traj.shape)   （7，2）
...
class BEVRender_Collision:
    def __init__(
        ...
        self.xlim = xlim
        self.ylim = ylim
        self.pred_dir = os.path.join(out_dir, "collision_bev_pred")
        os.makedirs(self.pred_dir, exist_ok=True)
    ...
    def render(
        ...
        self.reset_canvas()
        self.draw_detection_gt(fut_boxes, collision_index)
        self.draw_planning_gt(mask, final_planning)
        self._render_sdc_car()
        self._render_legend()
        self.axes.text(-38, -25, "index_t: ({}, {})".format(index, t), fontsize=40)
        save_path_pred = os.path.join(
            self.pred_dir, str(index).zfill(4) + "_" + str(t) + ".jpg"
        )
        self.save_fig(save_path_pred)
    ...
    def draw_detection_gt(self, fut_boxes, collision_index):
        for i in range(fut_boxes.shape[0]):
            color = color_mapping[i % len(color_mapping)]

            # draw corners
            corners = box3d_to_corners(fut_boxes)[i, [0, 3, 7, 4, 0]]
            x = corners[:, 0]
            y = corners[:, 1]
            self.axes.plot(x, y, color=color, linewidth=3, linestyle="-")
    ...
    def draw_planning_gt(self, masks, final_planning):
        # masks = masks.bool()
        # if masks[0] != 0:
        plan_traj = final_planning  # [masks]
        plan_traj[abs(plan_traj) < 0.01] = 0.0
        plan_traj = plan_traj.cumsum(axis=0)
        plan_traj = np.concatenate(
            (np.zeros((1, plan_traj.shape[1])), plan_traj), axis=0
...
class BEVRenderCustom:
    ...
    def render(
        ...
        # draw preds with gts
        self.draw_planning_gt(data)
        self.draw_planning_pred(data, result, top_k=self.plot_choices["planning_top_k"], colormap=colormap)
        # # draw preds with gts
        # self.draw_planning_gt(data)
        self._render_sdc_car()
        # self._render_command(data)
