...
from tools.visualization.custom_bev_render import BEVRender, BEVRender_Origin, BEVRenderCustom, BEVRenderCustom_Origin
...
class PlanningMetric():
    ...
    def evaluate_coll(self, trajs, gt_trajs, fut_boxes):
        ...
        for i in range(B):
            gt_box_coll, gt_collision_index = self.evaluate_single_coll(gt_trajs[i], fut_boxes)
            box_coll, collision_index = self.evaluate_single_coll(trajs[i], fut_boxes)
            box_coll = torch.logical_and(box_coll, torch.logical_not(gt_box_coll))  # 排除gt中已经发生碰撞的部分，只保留预测轨迹新增加的碰撞
    ...
    def update(self, trajs, gt_trajs, gt_trajs_mask, fut_boxes):
        ...
        L2 = self.compute_L2(trajs, gt_trajs, gt_trajs_mask)   # 这3个元素的shape都是6x2，显示的是未来6个时间步的轨迹坐标
        # fut_boxes是一个长度为6的list，显示的是未来6个时间步的boxes，在每个时间步，与任何一个boxes发生碰撞都视为发生碰撞
        obj_coll_sum, obj_box_coll_sum, collision_index = self.evaluate_coll(trajs[:,:,:2], gt_trajs[:,:,:2], fut_boxes)
...
class Visualizer:
    def __init__(
        ...
        self.sparsedrive_results = mmcv.load("work_dirs/sparsedrive_small_stage2/results.pkl") # load sparsedrive的results
        ...
        # self.bev_render_collision = BEVRender_Collision(plot_choices, self.out_dir)
    ...
    def add_vis(self, index):
        ...
        sdc_planning = torch.from_numpy(data['gt_ego_fut_trajs']).unsqueeze(0).cumsum(dim=-2).unsqueeze(1)
        sdc_planning_mask = torch.from_numpy(data['gt_ego_fut_masks']).unsqueeze(-1).repeat(1, 1, 2).unsqueeze(1)
        ...
        if sdc_planning_mask.all(): ## for incomplete gt, we do not count this sample
            ...
            sparsedrive_pred_sdc_traj = sparsedrive_result['final_planning'].unsqueeze(0)
            L2, obj_coll_sum, obj_box_coll_sum, collision_index = planning_metrics.update(pred_sdc_traj[:, :6, :2], sdc_planning[0,:, :6, :2], sdc_planning_mask[0,:, :6, :2], fut_boxes)
            sparsedrive_L2, sparsedrive_obj_coll_sum, sparsedrive_obj_box_coll_sum, sparsedrive_collision_index = planning_metrics.update(sparsedrive_pred_sdc_traj[:, :6, :2], sdc_planning[0,:, :6, :2], sdc_planning_mask[0,:, :6, :2], fut_boxes)
            if self.select_better and (L2 > sparsedrive_L2 or obj_box_coll_sum.mean() > sparsedrive_obj_box_coll_sum.mean()):
                return
            # bev_gt_path, bev_pred_path, _, _, _, _ = self.bev_render.render(data, result, index, L2)
            bev_pred_path, _, _, _, _ = self.bev_render.render(data, result, index, L2, colormap="autumn")
            sparsedrive_bev_pred_path = self.bev_render_origin.render(data, sparsedrive_result, index, sparsedrive_L2, colormap="winter")
            # cam_pred_path = self.cam_render.render(data, result, index)
            cam_pred_path = self.cam_render.render(data, result, sparsedrive_result, index)
            #self.combine(bev_gt_path, bev_pred_path, cam_pred_path, index)
            # self.combine_ours(bev_gt_path, bev_pred_path, cam_pred_path, sparsedrive_bev_pred_path, index)
            self.combine_custom(bev_pred_path, cam_pred_path, sparsedrive_bev_pred_path, index)
    ...
    def combine_ours(self, bev_gt_path, bev_pred_path, cam_pred_path, sparsedrive_bev_pred_path, index):
        bev_gt = cv2.imread(bev_gt_path)
        bev_image = cv2.imread(bev_pred_path)
        cam_image = cv2.imread(cam_pred_path)
    ...
    def combine_custom(self, bev_pred_path, cam_pred_path, sparsedrive_bev_pred_path, index):
        bev_image = cv2.imread(bev_pred_path)
        cam_image = cv2.imread(cam_pred_path)
        sparsedrive_bev_image = cv2.imread(sparsedrive_bev_pred_path)
        ...
        if h != target_h:
            ...
            cam_image = cv2.resize(cam_image, (target_w, target_h), interpolation=cv2.INTER_LINEAR)

        merge_image = cv2.hconcat([cam_image, bev_image, sparsedrive_bev_image])
        save_path = os.path.join(self.combine_dir, str(index).zfill(4) + '.jpg')
        cv2.imwrite(save_path, merge_image)
...
def main():
    ...
    for idx in tqdm(range(START, END, INTERVAL)):  # 0-81，索引间隔是1
        # if idx != 1762:
        #     continue
        if idx > len(visualizer.results): 
            break
        visualizer.add_vis(idx)   # 通过这一步得到图片和整合的image结果并保存到相关的路径
        # print(visualizer.dataset.get_data_info(idx)["img_filename"])
