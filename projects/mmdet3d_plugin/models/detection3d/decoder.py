...
class SparseBox3DDecoder(object):
    ...
    def decode(
        ...
        # UniUncer: Gather predicted quality tensor (includes Laplace stds) for top-k outputs.
        output_quality = quality[output_idx]
        ...
        for i in range(bs):
            ...
            box_quality = output_quality[i, indices[i] // num_cls]
            if self.score_threshold is not None:
                category_ids = category_ids[mask[i]]
                scores = scores[mask[i]]
                box = box[mask[i]]
                box_quality = box_quality[mask[i]]
            if quality is not None:
            ...
            output.append(
                {
                    ...
                    # UniUncer: Return predicted box quality (Laplace stds) for uncertainty-aware downstream use.
                    "quality_3d": box_quality.cpu(),
                }
            )
            if quality is not None:
