...
class Sparse4DHead(BaseModule):
    ...
    def forward(
        ...
        for i, op in enumerate(self.operation_order):
            if self.layers[i] is None:
                ...
                # UniUncer: For map tasks, pass the uncertainty tensor qt directly into regression loss.
                if self.task_prefix == 'map':
                    anchor, cls, qt = self.layers[i](    # anchor, cls, qt, velo =
                        instance_feature,
                        anchor,
                        anchor_embed,
    ...
    def loss(self, model_outs, data, feature_maps=None):
        ...
        if self.task_prefix == 'map':   # changed
            for decoder_idx, (cls, reg, qt) in enumerate(
                ...
                b,nq,n = reg.shape
                mask = mask.reshape(-1)
                temp_reg_weights=[1.0] * 40,
                reg_weights = reg_weights * reg.new_tensor(temp_reg_weights)   # changed
