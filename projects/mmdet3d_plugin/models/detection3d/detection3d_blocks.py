...
# UniUncer: Laplace branch for uncertainty-aware box regression.
# Predicts per-dimension means and positive stds (via softplus+clamp) for xyz, wlh, yaw, velocity.
class laplace_branch(nn.Module):
    def __init__(self, input_dim=256, hidden_dim=256, out_dim=10):
        super().__init__()
        ...
        self.fc_mean_xyz = nn.Linear(hidden_dim, 3)
        self.fc_std_xyz = nn.Linear(hidden_dim, 3)
        
        self.fc_mean_wlh = nn.Linear(hidden_dim, 3)
        self.fc_std_wlh = nn.Linear(hidden_dim, 3)
        
        self.fc_mean_yaw = nn.Linear(hidden_dim, 2)
        self.fc_std_yaw = nn.Linear(hidden_dim, 2)

        self.fc_mean_v = nn.Linear(hidden_dim, 3)
        self.fc_std_v = nn.Linear(hidden_dim, 3)


    def forward(self, x):
        h = self.layers(x)

        mean_xyz = self.fc_mean_xyz(h)
        mean_wlh = self.fc_mean_wlh(h)
        mean_yaw = self.fc_mean_yaw(h)
        mean_v = self.fc_mean_v(h)

        std_xyz = self.fc_std_xyz(h)
        std_wlh = self.fc_std_wlh(h)
        std_yaw = self.fc_std_yaw(h)
        std_v = self.fc_std_v(h)


        stds_xyz = torch.clamp(F.softplus(std_xyz), min=1e-3, max=10)
        stds_wlh = torch.clamp(F.softplus(std_wlh), min=1e-3, max=10)
        stds_yaw= torch.clamp(F.softplus(std_yaw), min=1e-3, max=10)
        stds_v = torch.clamp(F.softplus(std_v), min=1e-3, max=10)

        stds = torch.cat((stds_xyz, stds_wlh, stds_yaw, stds_v), -1)
        means = torch.cat((mean_xyz, mean_wlh, mean_yaw, mean_v), -1)
...
# UniUncer: Refinement module now uses laplace_branch to output (anchor, cls, qt=stds)
# instead of a single deterministic regression head.
class SparseBox3DRefinementModule(BaseModule):
    def __init__(
        ...
        # )
        # UniUncer: Replace deterministic MLP with Laplace branch for uncertainty-aware regression.
        self.layers = laplace_branch(embed_dims, embed_dims, output_dim)
    ...
    def forward(
        ...
        output, stds = self.layers(feature)
        ...
        if self.output_dim > 8:
            ...
            translation_stds = torch.transpose(stds[..., VX:], 0, -1)
            velocity_stds = torch.transpose(translation_stds / time_interval, 0, -1)
            stds[..., VX:] = velocity_stds
        ...
        if return_cls and self.with_quality_estimation:
            ...
            # concate stds of uncertainty with the original quality estimation
            quality = torch.cat((stds, quality), dim=-1)
        else:
            quality = None
