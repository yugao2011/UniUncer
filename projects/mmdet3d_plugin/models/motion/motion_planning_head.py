...
def topkUncer(confidence, k, uncer):
    bs, N = confidence.shape[:2]
    confidence, indices = torch.topk(confidence, k, dim=1)
    
    top_uncer = uncer.flatten(end_dim=1)[indices].reshape(bs,k,-1)

    return top_uncer

# UniUncer: Uncertainty-aware gate that modulates historical temporal features
# based on current static+dynamic uncertainty levels via attention-weighted gating.
class UncerStatusGater(nn.Module):
    def __init__(self, uncer_dim, num_history_steps=4):

        super().__init__()
        self.attn_fc = nn.Linear(uncer_dim, 1)
        self.gate_fc = nn.Linear(uncer_dim, num_history_steps)

        nn.init.constant_(self.gate_fc.bias, 1.0)

    def forward(self, uncer, status):
        B, _ = uncer.shape[:2]
        total, t, d = status.shape
        N = int(total/B)
        status_batched = status.reshape(B, N, t, d) 

        # map every object in a batch to 1
        attn_scores = self.attn_fc(uncer)
        # change all obj in a batch to a prob
        attn_weights = F.softmax(attn_scores, dim=1)
        # weight all obj in a batch and sum
        semantic_global = (uncer * attn_weights).sum(dim=1)
        full_gate = torch.sigmoid(self.gate_fc(semantic_global))

        gate = full_gate[:, :t]
        gate = gate.unsqueeze(1).unsqueeze(-1)

        masked_status = status_batched * gate
        output = masked_status.flatten(0, 1)
...
class MotionPlanningHead(BaseModule):
    def __init__(
        ...
        self.with_ucer = with_ucer  # UniUncer: flag to enable uncertainty branches
        ...
        self.op_config_map = {
            "ucer_map": [graph_model, ATTENTION],  # UniUncer: cross-attn using map uncertainty-aware queries
            "temp_gnn": [temp_graph_model, ATTENTION],
            "gnn": [graph_model, ATTENTION],
            "cross_gnn": [cross_graph_model, ATTENTION],
            "norm": [norm_layer, NORM_LAYERS],
            "ffn": [ffn, FEEDFORWARD_NETWORK],
            "ucer_det": [graph_model, ATTENTION],  # UniUncer: cross-attn using det uncertainty-aware queries
        ...
        if self.with_ucer:
            # UniUncer: Build encoder/fuser networks to embed Laplace stds into queries.
            # det and map quality encoder-
            self.det_ucer_encoder = nn.Sequential(
                    *linear_relu_ln(embed_dims, 1, 2, 3),
                    Linear(embed_dims, embed_dims),
                )
            
            self.det_ucer_fuser = nn.Sequential(
                    *linear_relu_ln(embed_dims, 1, 2, embed_dims*2),
                    Linear(embed_dims, embed_dims),
                )
            
            self.map_ucer_encoder = nn.Sequential(
                    *linear_relu_ln(embed_dims, 1, 2, 40),
                    Linear(embed_dims, embed_dims),
                )
            
            self.map_ucer_fuser = nn.Sequential(
                    *linear_relu_ln(embed_dims, 1, 2, embed_dims*2),
                    Linear(embed_dims, embed_dims),
                )
        ...
        # UniUncer: Gate module that modulates temporal history with aggregated uncertainty features.
        self.uncer_gate = UncerStatusGater(uncer_dim=embed_dims, num_history_steps=4)
    ...
    def forward(
        ...
        # print(f'temp instance feature shape is {temp_instance_feature.shape}')
        # print(f'temp instance mask shape is {temp_mask.shape}')
        ...
        # det ucertainty
        det_quality = det_output["quality"][-1]
        uncer_idx = [0,1,2]
        det_quality = det_quality[..., uncer_idx]

        b,nq,d = det_quality.shape
        ego_uncer = torch.zeros(b, 1, d).to(det_quality.device)

        det_ucer = torch.cat([det_quality,ego_uncer], dim=1)
        # UniUncer: Encode det Laplace stds and fuse with instance features to form uncertainty-aware queries.
        encode_det_ucer = self.det_ucer_encoder(det_ucer)
        instance_feature_ucer = self.det_ucer_fuser(torch.cat([encode_det_ucer, instance_feature], dim=-1))

        # map ucertainty
        # map_uncer_instance = torch.cat([map_instance_feature_selected, ego_feature], dim=1)
        map_quality = map_output["quality"][-1]
        map_quality_selected = topkUncer(map_confidence,self.num_map,map_quality)
        # UniUncer: Encode map Laplace stds and fuse with map instance features.
        encode_map_ucer_selected= self.map_ucer_encoder(map_quality_selected)
        map_instance_feature_ucer_selected = self.map_ucer_fuser(torch.cat([encode_map_ucer_selected, map_instance_feature_selected], dim=-1))

        anchor_embed = torch.cat([anchor_embed, ego_anchor_embed], dim=1)

        # uni uncer gate
        uni_uncer_feature = torch.cat([instance_feature_ucer, map_instance_feature_ucer_selected], dim=1)
        # print(f'before mask temp_instance_feature shape is {temp_instance_feature.shape}')
        temp_instance_feature = self.uncer_gate(uni_uncer_feature, temp_instance_feature)
        # print(f'masked temp_instance_feature shape is {temp_instance_feature.shape}')

        # anchor_embed_ucer = self.det_ucer_anchor_fuser(torch.cat([encode_det_ucer, anchor_embed], dim=-1))
        ...
        for i, op in enumerate(self.operation_order):
            if self.layers[i] is None:
                continue
            # UniUncer: Map uncertainty-aware cross-attention — map queries attend to uncertainty-fused map features.
            elif op == "ucer_map":
                map_instance_feature_selected = self.graph_model(
                    i,
                    map_instance_feature_selected,
                    map_instance_feature_ucer_selected,
                    map_instance_feature_ucer_selected,
                    query_pos=map_anchor_embed_selected,
                    key_pos=encode_map_ucer_selected,
                )
            elif op == "temp_gnn":
            ...
            # UniUncer: Det uncertainty-aware self-attention — instance features attend to uncertainty-fused det features.
            elif op == "ucer_det":
                instance_feature = self.graph_model(
                    i,
                    instance_feature,
                    instance_feature_ucer,
                    instance_feature_ucer,
                    query_pos=anchor_embed,
                    key_pos=encode_det_ucer,
                )
            elif op == "refine":
                motion_query = motion_mode_query + (instance_feature + anchor_embed + encode_det_ucer)[:, :num_anchor].unsqueeze(2)
                plan_query = plan_mode_query + (instance_feature + anchor_embed + encode_det_ucer)[:, num_anchor:].unsqueeze(2)
                (
                    motion_cls,
                    motion_reg,
                    plan_cls,
