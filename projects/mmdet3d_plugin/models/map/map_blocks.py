...
# UniUncer: Laplace branch for uncertainty-aware map point regression.
# Predicts per-point means and positive stds (via softplus+clamp) for sampled 2D coordinates.
class laplace_branch(nn.Module):
    def __init__(self, input_dim=256, hidden_dim=256, num_sample=20, coords_dim=2):
        super().__init__()
        ...
        self.fc_xy_means = nn.Linear(hidden_dim, 2*self.num_sample)
        self.fc_xy_stds = nn.Linear(hidden_dim, 2*self.num_sample)

    def forward(self, x):
        h = self.layers(x)
        means = self.fc_xy_means(h)
        raw_stds = self.fc_xy_stds(h)
        stds = torch.clamp(F.softplus(raw_stds), min=1e-3, max=5) 
...
# UniUncer: Refinement module now uses laplace_branch to output (means, stds) for map points.
class SparsePoint3DRefinementModule(BaseModule):
    def __init__(
        ...
        # UniUncer: Replace deterministic MLP with Laplace branch for uncertainty-aware map regression.
        self.layers = laplace_branch(self.embed_dims, self.embed_dims, self.num_sample, self.coords_dim)
    ...
    def forward(
        ...
        means, stds = self.layers(instance_feature + anchor_embed)
        b,nq,n = means.shape
