...
class SparsePoint3DDecoder(object):
    ...
    def decode(
        ...
        output_quality = quality[-1].reshape(bs, num_pred, -1, self.coords_dim)
        ...
        for i in range(bs):
            ...
            pts_quality = output_quality[i, indices[i] // num_cls]

            if self.score_threshold is not None:
                category_ids = category_ids[mask[i]]
                scores = scores[mask[i]]
                pts = pts[mask[i]]
                pts_quality = pts_quality[mask[i]]
            output.append(
                {
                    "vectors": [vec.detach().cpu().numpy() for vec in pts],
                    # UniUncer: Return predicted map point quality (Laplace stds) for uncertainty-aware downstream use.
                    "vectors_quality": [vec.detach().cpu().numpy() for vec in pts_quality],
