...
class SparsePoint3DTarget(BaseTargetWithDenoising):
    ...
    def sample(
        ...
        # print('pts target shape',pts_targets[0].shape) # n,38,20,2
        # flatten the reg gt to 20,2
        # UniUncer: Flatten map point targets to match the new prediction format (means+stds).
        ...
        '''# add velo pred and ego status for expand map elements to x,y,vx,vy
        for(cls_pred, pts_pred, velo_pred, cls_target, pts_target, ego_status) in zip(
            cls_preds, pts_preds, velo_preds, cls_targets, pts_targets, ego_targets
        ):
            # normalize to (0, 1)
            pts_pred = self.normalize_line(pts_pred)
            pts_target = self.normalize_line(pts_target)
            # print('assigner gt shape',pts_target.shape) 100,40
            # print('assigner preds shape',pts_pred.shape) n,38,40
            preds=dict(lines=pts_pred, scores=cls_pred)
            gts=dict(lines=pts_target, labels=cls_target)
            indice = self.assigner.assign(preds, gts)
            indices.append(indice)
        ...
        # handle velo for map
        B,NA,_ = pts_preds.shape
        pts_preds_reshaped = pts_preds.view(B, NA, 20, 2)
        vx = velo_preds[:, :, 0].unsqueeze(2)
        vy = velo_preds[:, :, 1].unsqueeze(2)
        vx_expanded = vx.expand(B, NA, 20)
        vy_expanded = vy.expand(B, NA, 20)
        vxy_expanded = torch.stack((vx_expanded, vy_expanded), dim=3)
        new_pts_preds = torch.cat((pts_preds_reshaped, vxy_expanded), dim=3)
        # now the pts preds become (b, na, 80)
        new_pts_preds = new_pts_preds.view(B, NA, -1)

        bs, num_pred, num_cls = cls_preds.shape
        output_cls_target = cls_targets[0].new_ones([bs, num_pred], dtype=torch.long) * num_cls
        # output_box_target = pts_preds.new_zeros(pts_preds.shape)
        output_reg_weights = new_pts_preds.new_zeros(new_pts_preds.shape)
        output_box_target = new_pts_preds.new_zeros(new_pts_preds.shape)
        ...
        for i, (pred_idx, target_idx, gt_permute_index) in enumerate(indices):
            ...
            # add ego status to map targets
            # print('pts target shape',pts_targets[i].shape) m,38,40
            m,n,l = pts_targets[i].shape
            pts_targets_reshaped = pts_targets[i].view(m, n, 20, 2)
            # map velo is -v_ego
            vx = ego_targets[i][0]*-1
            vy = ego_targets[i][1]*-1
            vx_tensor = vx * torch.ones((m, n, 20, 1),device=vx.device)
            vy_tensor = vy * torch.ones((m, n, 20, 1),device=vx.device)
            new_pts_targets = torch.cat((pts_targets_reshaped, vx_tensor, vy_tensor), dim=3)
            new_pts_targets = new_pts_targets.view(m, n, -1)

            output_box_target[i, pred_idx] = new_pts_targets[target_idx, permute_idx]
            output_reg_weights[i, pred_idx] = 1

        # print('assigned target shape', output_box_target.shape)
        return output_cls_target, output_box_target, output_reg_weights'''
