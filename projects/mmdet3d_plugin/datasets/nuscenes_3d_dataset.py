...
class NuScenes3DDataset(Dataset):
    ...
    def _set_sequence_group_flag(self):
        ...
        # UniUncer: np.int -> np.int64 to fix numpy deprecation warning.
    ...
    def evaluate(
        ...
        if eval_mode['with_planning']:
            ...
            planning_results_dict, metrics_list = planning_eval(results, self.eval_config, logger=logger)
            # UniUncer: Save per-sample planning metrics to pickle for detailed analysis.
            metrics_path = "metrics.pkl"
            metrics_path = osp.join(self.work_dir, metrics_path)
            print("All Results write to", metrics_path)
            mmcv.dump(metrics_list, metrics_path)
