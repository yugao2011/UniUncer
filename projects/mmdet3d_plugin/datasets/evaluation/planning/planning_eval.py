...
from mmengine import print_log  # UniUncer: replaced deprecated mmcv.print_log
...
class PlanningMetric():
    ...
    def update(self, trajs, gt_trajs, gt_trajs_mask, fut_boxes):
        ...
        return {"L2": L2, "obj_coll_sum": obj_coll_sum, "obj_box_coll_sum": obj_box_coll_sum}
...
def planning_eval(results, eval_config, logger):
    ...
    # UniUncer: Collect per-sample planning metrics for detailed analysis.
    metrics_list = []
    for i, data in enumerate(tqdm(dataloader)):
        ...
        metrics_list.append(planning_metrics.update(pred_sdc_traj[:, :6, :2], sdc_planning[0,:, :6, :2], sdc_planning_mask[0,:, :6, :2], fut_boxes))
    ...
    # UniUncer: Return both aggregated metrics and per-sample metrics list.
    return metric_dict, metrics_list
